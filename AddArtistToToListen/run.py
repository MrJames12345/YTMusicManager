...
with open('artist.txt') as f:
    searchArtistName = f.readlines()[0].title();


print('\nArtist: "' + searchArtistName + '"');


# Get artist's browseId from yt music search
artistList = api.search(searchArtistName, "artists");
for tempArtist in artistList:
    if tempArtist['artist'].lower() == searchArtistName.lower():
        browseId = tempArtist['browseId'];
        break;


# IF didn't find artist, exit
try: browseId
except NameError:
